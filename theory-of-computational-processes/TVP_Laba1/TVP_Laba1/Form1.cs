namespace TVP_Laba1
{
    public partial class Form1 : Form
    {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            historyTextBox.Text += $"Перевели число А в двоичный вид прямой код  - {RgA.BitsToString()}\n";
            DecimalToBinary(RgB, Convert.ToInt32(textBoxDecimalB.Text));
            historyTextBox.Text += $"Перевели число B в двоичный вид прямой код - {RgA.BitsToString()}\n";
            ...
            {
                ToAdditionalCode(RgA);
                historyTextBox.Text += $"Перевели отрицательное число А в дополнительный код - {RgA.BitsToString()}\n";
            }
                ...
                RgB.GetBit(RgB.GetSize() - 1) == 0 && radioButtonSub.Checked)
            {
                ...
                ToAdditionalCode(RgB);
                historyTextBox.Text += $"Перевели отрицательное число B в дополнительный код - {RgB.BitsToString()}\n";
            }
            ...
            {
                historyTextBox.Text += $"В мультиплексоры загружена {i + 1} часть регистров А и B...\n";
                ...
                historyTextBox.Text += $"{i + 1} часть результата выгружена из сумматора...\n\n";
                Ct += 2;
            }
            demull.SetBits(RgC);
        ...
        private static void ToAdditionalCode(Register reg)
        {
            int transferSing = 1;
